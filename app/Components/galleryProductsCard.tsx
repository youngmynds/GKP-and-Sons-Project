    ...
    return (
        <div className="max-w-7xl mx-auto bg-black text-center mb-5 w-[calc(50%-16px)] sm:w-[calc(30%-12px)] md:w-[calc(30%-12px)]">
            <button onClick={onClick} className="w-full h-full flex flex-col">
                <div className="relative w-full h-[200px] md:h-[350px]"> {/* Fixed height */}
                    <Image
                        src={src}
                        alt={title}
                    ...
                    />
                    <p className={`absolute bottom-0 left-0 uppercase w-full bg-black text-white py-2 my-0 sm:text-lg md:text-xl ${montserrat.className}`}>
                        {title}
                    </p>
                </div>
            </button>
