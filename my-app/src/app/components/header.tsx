    ...
    <div className="relative">
      <header className="flex items-center justify-between p-6 bg-[#170722] w-full">
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 md:relative md:left-0 md:transform-none">
          <Image src="/GKP_Logo.png" alt="Logo" width={170} height={150} />
        </div>

        {/* Desktop Navigation */}
        <nav className={`hidden md:flex gap-8 ml-20 ${montserrat.className} justify-center`}>
          {["Home", "About Us", "Products", "Collections", "Contact Us"].map(
            (item, index) => (
              <a
                href="#"
                key={index}
                className={`hover:underline text-base tracking-wide ${item === "Collections" ? "text-gold" : "text-white"
                  }`}
              >
                {item}
              </a>
        ...
        <div className="hidden md:flex items-center justify-center space-x-3">
          <p className={`text-gold text-xl ${parisienne.className}`}>Call Us</p>
          <p className={`text-white ${montserrat.className}`}>+91 98765 43210</p>
        </div>
      </header>
